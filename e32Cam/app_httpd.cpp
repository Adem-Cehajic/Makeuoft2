...
static esp_err_t index_handler(httpd_req_t *req) {
  // Minimal page: no UI, starts streaming immediately.
  httpd_resp_set_type(req, "text/html");

  // Stream server in this example runs on port 81 (config.server_port += 1).
  // We point the <img> directly to the MJPEG stream.
  const char *html =
      ...
      "#s{position:fixed;inset:0;width:100%;height:100%;object-fit:contain;background:#000;}"
      "</style>"
      "</head><body>"
      "<img id='s' alt='stream' />"
      "<script>"
